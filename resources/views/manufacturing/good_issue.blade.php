                                        ...
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                ...
                                                <div class="col-lg-4">
                                                    ...
                                                    @php
                                                        $svc= App\Models\Service::where('status','=','0')->get();
                                                        @endphp
                                                          @foreach($svc as $s)

                                                          <option @if(isset($data))
                                                              {{ $data->type_id == $s->id  ? 'selected' : ''}}
                                                              @endif value="{{$s->id}}">{{$s->truck_name}}</option> 
                                                          @endforeach

                                                       @elseif($data->type == 'Maintenance')
                                                       @php
                                                        $mn= App\Models\Maintainance::where('status','=','0')->get(); 
                                                        @endphp
                                                          @foreach( $mn as $m)

                                                          <option @if(isset($data))
                                                              {{ $data->type_id == $m->id  ? 'selected' : ''}}
                                                              @endif value="{{$m->id}}">{{$m->truck_name}}</option> 
                                                          @endforeach
