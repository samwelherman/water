                            ...
                            <div class="tab-pane fade @if(empty($id)) active show @endif" id="home2" role="tabpanel"
                                ...
                                <div class="table-responsive">
                                 ...
                                 <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                   ...
                                   @if($row->collected == 0)
                                            <td>
 <a  class="btn btn-xs btn-outline-info text-uppercase px-2 rounded" title="Edit" onclick="return confirm('Are you sure?')" href="{{ route('pacel_quotation.edit', $row->id)}}"><i class="fa fa-edit"></i></a>
...
{{ Form::close() }}
      ...
      <li class="nav-item"><a  class="nav-link" title="Edit" data-toggle="modal" class="discount"  href="" onclick="model({{ $row->id }},'discount')" value="{{ $row->id}}" data-target="#appFormModal" >Discount Quotation</a></li>
          <li class="nav-item"><a  class="nav-link" title="Convert to Invoice" onclick="return confirm('Are you sure? you want to confirm collection')"  href="{{ route('pacel.approve', $row->id)}}">Collect Package</a></li>        
          <li class="nav-item"><a  class="nav-link" title="Cancel" onclick="return confirm('Are you sure?')"   href="{{ route('pacel.cancel', $row->id)}}">Cancel Quotation</a></li>                                       
                          </ul></div>
...
<?php $a=1;?>
                                                  ...
                                                  <button class="btn btn-primary" type="button" data-toggle="modal" onclick="model({{ $a }},'supplier')" value="{{ $a}}" data-target="#appFormModal"><i class="fa fa-plus-circle"></i></button>
                                                  </div>
                                                 </div>
                                                    </div>
                                                </div>
...
@if(!empty($id))
   @if(!@empty($items))
    @foreach ($items as $i)
 <tr class="line_items">
<td><select name="item_name[]" class="form-control item_name" required  data-sub_category_id={{$i->order_no}}><option value="">Select Item</option>@foreach($route as $n) <option value="{{ $n->id}}" @if(isset($i))@if($n->id == $i->item_name) selected @endif @endif >From {{$n->from}} to {{$n->to}} ({{$n->distance}} km)</option>@endforeach</select></td>
...
<td><select name="charge[]" class="form-control item_charge'+count{{$i->order_no}}" required >
<option value="0">Select Charge Type</option>
<option value="1" @if(isset($i))@if('Flat' == $i->charge_type) selected @endif @endif>Flat</option>
<option value="{{$i->distance}}"  name="one" @if(isset($i))@if('Distance' == $i->charge_type) selected @endif @endif>Distance per ton</option></select>
</td>
<td><select name="tax_rate[]" class="form-control item_tax'+count{{$i->order_no}}" required >
...
<script>
    ...
    $('.add').on("click", function(e) {
        ...
        html += '<td><select name="item_name[]" class="form-control item_name" required  data-sub_category_id="'+count+'"><option value="">Select Route</option>@foreach($route as $n) <option value="{{ $n->id}}">From {{$n->from}} to {{$n->to}}  ({{$n->distance}} km)</option>@endforeach</select></td>';
       ...
       html += '<td><input type="text" name="unit[]" class="form-control item_unit'+count+'" placeholder ="unit" required /></td>';
            html += '<td><select name="charge[]" class="form-control item_charge'+count+'" required   data-sub_category_id="'+count+'" ><option value="0">Select Charge Type</option><option value="1">Flat</option><option value="" name="one">Distance per ton</option></select></td>';
