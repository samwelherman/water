...
class FontMetrics
{
    ...
    public function saveFontFamilies()
    {
        ...
        foreach ($this->fontLookup as $family => $variants) {
            ...
            foreach ($variants as $variant => $path) {
                $path = sprintf("'%s'", $path);
                $path = str_replace('\'' . $this->config_items()->getFontDir() , '$fontDir . \'' , $path);
                $path = str_replace('\'' . $this->config_items()->getRootDir() , '$rootDir . \'' , $path);
    ...
    {
        $fontDir = $this->config_items()->getFontDir();
        $rootDir = $this->config_items()->getRootDir();
    ...
    {
        return $this->config_items()->getFontDir() . DIRECTORY_SEPARATOR . self::CACHE_FILE;
    }
