...
class GD implements Canvas
{
    ...
    public function __construct($size = 'letter', $orientation = "portrait", Dompdf $dompdf, $aa_factor = 1.0, $bg_color = array(1, 1, 1, 0))
    {
        ...
        $this->dpi = $this->get_dompdf()->config_items()->getDpi();
    ...
    {
        $this->get_dompdf()->config_items()->setDpi(72);
    }
    ...
    {
        $this->get_dompdf()->config_items()->setDpi($this->dpi);
    }
    ...
    public function get_ttf_file($font)
    {
        ...
        if (!file_exists($font)) {
            ...
            $font = $font_metrics->getFont($this->_dompdf->config_items()->getDefaultFont()) . ".ttf";
            if (!file_exists($font)) {
                if (strpos($font, "mono")) {
