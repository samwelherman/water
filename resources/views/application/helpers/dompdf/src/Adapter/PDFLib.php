...
class PDFLib implements Canvas
{
    ...
    public function __construct($paper = "letter", $orientation = "portrait", Dompdf $dompdf)
    {
        ...
        $license = $dompdf->config_items()->getPdflibLicense();
        if (strlen($license) > 0) {
        ...
        $searchPath = $this->_dompdf->config_items()->getFontDir();
        ...
        if (self::$IN_MEMORY) {
            ...
            $tmp_dir = $this->_dompdf->config_items()->getTempDir();
    ...
    protected function _load_font($font, $encoding = null, $options = "")
    {
        ...
        if (is_null($encoding)) {
            ...
            if (strlen($this->_dompdf->config_items()->getPdflibLicense()) > 0) {
                $encoding = "unicode";
            } else {
                $encoding = "auto";
            }
    ...
    public function get_font_height($font, $size)
    {
        ...
        // $desc is usually < 0,
        $ratio = $this->_dompdf->config_items()->getFontHeightRatio();
    ...
    public function stream($filename = "document.pdf", $options = array())
    {
        ...
        if (self::$IN_MEMORY) {
            ...
            //debugpng
            if ($this->_dompdf->config_items()->getDebugPng()) {
                print '[pdflib stream unlink ' . $this->_file . ']';
            }
            if (!$this->_dompdf->config_items()->getDebugKeepTemp()) {
                unlink($this->_file);
            }
            $this->_file = null;
            unset($this->_file);
