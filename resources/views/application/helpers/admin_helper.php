...
define("CLIENT_CURRENCIES", get_all_currencies());
define("DEFAULT_CURRENCY", set_default_currency());
...
//    foreach ($currencies_res as $v) {
//        $currencies[$v->client_id] = $v->symbol;
//    }

    return serialize(array_column($currencies_res, 'symbol', 'client_id'));
...
{
    $currencies = unserialize(CLIENT_CURRENCIES);
...
function update_module_tags($data)
{
    ...
    if (is_array($input_tags)) {
        ...
        // already_tags
//        $tags = already_tags($table, $id);

//        $all_diff_tag = array_diff($input_tags, $tags); // get input tag tag from applied tag
//        if (!empty($tags)) {
//            $all_remove_tag = array_diff($tags, $input_tags); // after insert change the tag when update
//        }
//
//        $adata['tags'] = $data;
//        update($table, array('project_id' => $id), $adata);

        // get all tag which is not already applied
//        $tname = '';
//        if (!empty($all_diff_tag)) {
//            foreach ($all_diff_tag as $diff_tag) {
//                $tname .= $diff_tag . ',';
//            }
//            $adata['tags'] = $data;
//            update($table, array('project_id' => $id), $adata);
//        }
        // get all tag which is not already applied
//        if (!empty($all_remove_tag)) {
//            foreach ($all_remove_tag as $remove_tag) {
//                if (!empty($remove_tag)) {
//                    $r_tag_id = get_row('tbl_tags', array('name' => $remove_tag))->tag_id;
//                    if (!empty($r_tag_id)) {
//                        delete('tbl_tags', array('module_id' => $id, 'module' => $type, 'tag_id' => $r_tag_id));
//                    }
//                }
//            }
//        }
