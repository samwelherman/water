...
class elFinder
{
    ...
    protected function open($args)
    {
        ...
        // long polling mode
        if ($args['compare']) {
            $sleep = max(1, (int)$volume->config_item('lsPlSleep'));
            $standby = (int)$volume->config_item('plStandby');
    ...
    protected function file($args)
    {
        ...
        if ($download || $onetime) {
            ...
            $dispInlineRegex = $volume->config_item('dispInlineRegex');
            $inlineRegex = false;
            if ($dispInlineRegex) {
        ...
        if (!$onetime) {
            ...
            if ($cacheHeaders = $volume->config_item('cacheHeaders')) {
                $result['header'] = array_merge($result['header'], $cacheHeaders);
            }

            // check 'xsendfile'
            $xsendfile = $volume->config_item('xsendfile');
            $path = null;
            if ($xsendfile) {
                $info = stream_get_meta_data($fp);
                if ($path = empty($info['uri']) ? null : $info['uri']) {
                    $basePath = rtrim($volume->config_item('xsendfilePath'), DIRECTORY_SEPARATOR);
                    if ($basePath) {
        ...
        if ($mime && (strtolower(substr($mime, 0, 4)) === 'text' || in_array(strtolower($mime), self::$textMimes))) {
            ...
            if ($content !== '') {
                if (!$args['conv'] || $args['conv'] == '1') {
                    ...
                    if ($enc === 'unknown') {
                        $enc = $volume->config_item('encoding');
    ...
    protected function info($args)
    {
        ...
        if ($args['compare'] && count($args['targets']) === 1) {
            ...
            if ($volume = $this->volume($hash)) {
                $standby = (int)$volume->config_item('plStandby');
                $_compare = false;
                if (($syncCheckFunc = $volume->config_item('syncCheckFunc')) && is_callable($syncCheckFunc)) {
                    $_compare = call_user_func_array($syncCheckFunc, array($volume->realpath($hash), $standby, $compare, $volume, $this));
                }
                if ($_compare !== false) {
                    $compare = $_compare;
                } else {
                    $sleep = max(1, (int)$volume->config_item('tsPlSleep'));
