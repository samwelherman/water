...
class Report extends Admin_Controller
{
    ...
    public function bugs_assignment()
    {
    ...
    /************************19/08/2021***********Purchase Report*******************/
    #----------- $filterBy == purchase ---------------------------------------------
    public function purchase_report($filterBy = null)
    {
        if(isset($_GET['status'])){
            $status = $_GET['status'];
            $date = $_GET['date_ranger'];
        }
        $data['title'] = lang('purchase_report') . ' ' . lang('report');
        if (!empty($filterBy)) {
            $data['filterBy'] = $filterBy;
            if ($filterBy == 'purchases' || $filterBy == 'purchases_by_supplier') {
                $start_date = null;
                $end_date = null;
                if ($filterBy == 'purchase_by_supplier') {
                    $supplier = get_result('tbl_suppliers');
                    $status = !empty($supplier) ? $supplier[0]->supplier_id : '0';
                } else {
                    $status = 'all';
                }
                if ($this->input->post()) {
                ...
                $range = array($start_date, $end_date);
                $data['status'] = $status;
                $data['all_purchases'] = $this->purchase_model->get_purchase_report($data['status'], $range);
                ...
                $range = array($start_date, $end_date);
                $data['status'] = $client_id;
                $data['all_payments'] = $this->purchase_model->get_payment_report($data['status'], $range);
            } 
       
        }
        
        if(!empty($filterBy)){
            // then grab all purchase data from the sql
            $purchase = $this->db->select('*')->from('tbl_purchases')->get();
            echo "<pre>";
            var_dump($purchase); exit;
        }
        
        if(!empty($status) && !empty($date)){
            echo "<script>alert('Data is being sorted dude!')</script>";
        }
    ...
    public function creditor_report($filterBy = null)
    {
        $data['title'] = lang('creditor') . ' ' . lang('report');
        if (!empty($filterBy)) {
            $data['filterBy'] = $filterBy;
            if ($filterBy == 'invoices' || $filterBy == 'invoice_by_client') {
                $start_date = null;
                ...
                $range = array($start_date, $end_date);
                $data['status'] = $status;
                $data['all_invoices'] = $this->invoice_model->get_creditor_report($data['status'], $range);
        ...
        $data['subview'] = $this->load->view('admin/report/creditor_report', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
