...
// retrive all data from department table
    ...
    public function provident_fund_pdf($year, $month)
    {
        ...
        $this->load->helper('dompdf');
        $viewfile = $this->load->view('ad
        min/payroll/provident_fund_pdf', $data, TRUE);
...
public function nhif_info()
    {
        ...
        // get all expense list by year and month
        $data['nhif_info'] = $this->get_nhif_info($data['year']);

        $data['subview'] = $this->load->view('admin/payroll/nhif_contr', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }
   
   
    public function get_nhif_info($year, $month = NULL)
    {// this function is to create get monthy recap report
        if (!empty($month)) {
            if ($month >= 1 && $month <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                $start_date = $year . "-" . '0' . $month;
            ...
            $nhif_info = $this->payroll_model->get_nhif_info_by_date($start_date, $end_date); // get all report by start date and in date
        } else {
            for ($i = 1; $i <= 12; $i++) { // query for months
                if ($i >= 1 && $i <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                    $start_date = $year . "-" . '0' . $i;
                ...
                $nhif_info[$i] = $this->payroll_model->get_nhif_info_info_by_date($start_date, $end_date); // get all report by start date and in date
            }
        }

        return $nhif_info; // return the result
    }
	
public function wcf_info()
    {
        $data['title'] = "WCF";
        // active check with current month
        $data['current_month'] = date('m');
        ...
        // get all expense list by year and month
        $data['wcf_info'] = $this->get_nhif_info($data['year']);

        $data['subview'] = $this->load->view('admin/payroll/wcf_info', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }
   
   
    public function get_wcf_info($year, $month = NULL)
    {// this function is to create get monthy recap report
        if (!empty($month)) {
            if ($month >= 1 && $month <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                $start_date = $year . "-" . '0' . $month;
            ...
            $wcf_info = $this->payroll_model->get_wcf_info_by_date($start_date, $end_date); // get all report by start date and in date
        } else {
            for ($i = 1; $i <= 12; $i++) { // query for months
                if ($i >= 1 && $i <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                    $start_date = $year . "-" . '0' . $i;
                ...
                $wcf_info[$i] = $this->payroll_model->get_wcf_info_info_by_date($start_date, $end_date); // get all report by start date and in date
            }
        }

        return $wcf_info; // return the result
    }

public function tax_fund()
    {
        $data['title'] = "PAYE";
        // active check with current month
        $data['current_month'] = date('m');
        ...
        // get all expense list by year and month
        $data['tax_fund'] = $this->get_tax_fund($data['year']);

        $data['subview'] = $this->load->view('admin/payroll/tax_deduction_info', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }
   
   
    public function get_tax_fund($year, $month = NULL)
    {// this function is to create get monthy recap report
        if (!empty($month)) {
            if ($month >= 1 && $month <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                $start_date = $year . "-" . '0' . $month;
            ...
            $tax_fund_info = $this->payroll_model->get_tax_deduction_info_by_date($start_date, $end_date); // get all report by start date and in date
        } else {
            for ($i = 1; $i <= 12; $i++) { // query for months
                if ($i >= 1 && $i <= 9) { // if i<=9 concate with Mysql.becuase on Mysql query fast in two digit like 01.
                    $start_date = $year . "-" . '0' . $i;
                ...
                $tax_fund_info[$i] = $this->payroll_model->get_tax_deduction_info_by_date($start_date, $end_date); // get all report by start date and in date
            }
        }

        return $tax_fund_info; // return the result
    }
