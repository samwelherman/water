...
class Payroll_Model extends MY_Model
{
    ...
    /*************New Report Ipmlematation******************/
       public function get_nhif_info_info_by_date($start_date, $end_date, $user_id = null)
    {
        $this->db->select('tbl_salary_payment.*', FALSE);
        ...
        $this->db->where('tbl_salary_payment_allowance.salary_payment_allowance_label', 'Medical Allowance');
        $this->db->where('tbl_salary_payment_allowance.salary_payment_allowance_label', 'Medical Allowance' );
        if (!empty($user_id)) {
        ...
        public function get_wcf_info_by_date($start_date, $end_date, $user_id = null)
    {
        $this->db->select('tbl_salary_payment.*', FALSE);
        ...
        $this->db->where('tbl_salary_payment.payment_month >=', $start_date);
        $this->db->where('tbl_salary_payment.payment_month <=', $end_date);
        $this->db->where('tbl_salary_payment_deduction.salary_payment_deduction_label', 'WCF');
        $this->db->where('tbl_salary_payment_deduction.salary_payment_deduction_label', 'WCF' );
        if (!empty($user_id)) {
            $this->db->where('tbl_salary_payment.user_id', $user_id);
        }
  ...
  public function get_tax_deduction_info_by_date($start_date, $end_date, $user_id = null)
    {
        $this->db->select('tbl_salary_payment.*', FALSE);
        $this->db->select('tbl_salary_payment_deduction.*', FALSE);
        $this->db->select('tbl_account_details.*', FALSE);
         $this->db->select('tbl_salary_payment_details.*', FALSE);
        $this->db->from('tbl_salary_payment');
        $this->db->join('tbl_salary_payment_deduction', 'tbl_salary_payment_deduction.salary_payment_id  = tbl_salary_payment.salary_payment_id', 'left');
        $this->db->join('tbl_account_details', 'tbl_account_details.user_id  = tbl_salary_payment.user_id', 'left');
        $this->db->join('tbl_salary_payment_details', 'tbl_salary_payment_details.salary_payment_id = tbl_salary_payment.salary_payment_id', 'left');
       
        $this->db->where('tbl_salary_payment.payment_month >=', $start_date);
        $this->db->where('tbl_salary_payment.payment_month <=', $end_date);
        $this->db->where('tbl_salary_payment_deduction.salary_payment_deduction_label', lang('tax_deduction'));
        $this->db->where('tbl_salary_payment_deduction.salary_payment_deduction_label', lang('tax_deduction'));
        
       $this->db->where('tbl_salary_payment_details.salary_payment_details_label', lang('basic_salary'));
       $this->db->where('tbl_salary_payment_details.salary_payment_details_label', lang('basic_salary'));
