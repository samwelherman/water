...
<div class="">
    <div class="hidden-print">
        <div class="criteria-band">
            <address class="row">
                <?php echo form_open(base_url() . 'admin/report/purchase_report/' . $filterBy);
                if ($filterBy == 'purchase_by_supplier') {

                    ?>
                    <div class="col-md-3">
                        <label><?= lang('select_supplier') ?>333333</label>
                            ...
                            if (!empty($all_supplier)) {
                                foreach ($all_supplier as $v_supplier) {
                                    ...
                                    <option value="<?= $v_supplier->supplier_id ?>"
                                        <?php
                                        if (!empty($status)) {
                                            echo $status == $v_supplier->supplier_id ? 'selected' : null;
                                        }
                                        ?>
                                    ><?= ucfirst($v_supplier->name) ?></option>
                                    <?php
                                }
                            }
                            ?>
    ...
    <div class="rep-container">
        ...
        <th class="text-right">
            <div class=" "> <?= lang('supplier_amount') ?></div>
        </th>
        <th class="text-right">
       ...
       <th class="text-right">
            <div class=" "> <?= lang('paid_amount') ?></div>
        </th>
        ...
        if (!empty($all_purchases)) {
            foreach ($all_purchases as $key => $purchase) {
                ...
                $purchase_due = $this->purchase_model->calculate_to('purcharse_due', $purchase->purchase_id);
                $due_total += $purchase_due;
                
                 $paid_amount = $this->purchase_model->calculate_to('paid_amount', $purchase->purchase_id);
                ...
                <tr>
                    ...
                    <td class="text-right">
                        <?= display_money($purchase_payable, $cur->symbol); ?>
                        pppp
                        </td>
                 ...
                 <td class="text-right">
                        <?= display_money($paid_amount, $cur->symbol); ?></td>
            ...
            <tr class="hover-muted bt">
                ...
                <td class="text-right"><?= display_money($paid_amount_total, $cur->symbol) ?></td>
